...
public class StaffExplorerController implements Initializable {
    ...
    private void getEmployeeDate() {
        ...
        String query = "SELECT p.SSN, p.FName, p.LName, p.Social_Status, p.Phone_Number, p.Email, p.Date_of_Birth, p.Gender, "
        ...
        try (Connection conn = DriverManager.getConnection(url, user, password); var stmt = conn.createStatement(); var rs = stmt.executeQuery(query)) {
            ...
            while (rs.next()) {
                ...
                Staff staffMember = new Staff(ssn, "", gender, fname, lname, "", socialStatus, email, phoneNumber, dob, hireDate, expertise, role, salary);
    ...
    // Method to handle editing the selected employee (you can expand this logic)
    @FXML
    private void editEmployee() {
        Staff selectedEmployee = employeeTable.getSelectionModel().getSelectedItem();
        if (selectedEmployee != null) {
            // Here you would add logic to display a form for editing the employee's details
            // For now, we will just change the first name as an example
            selectedEmployee.setFName("Updated Name");
